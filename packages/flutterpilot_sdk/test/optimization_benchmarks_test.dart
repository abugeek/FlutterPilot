...
void main() {
  group('Optimization & Performance Benchmarks', () {
    testWidgets(
      'Single-Pass O(N) Matcher accurately finds multi-priority targets',
      (tester) async {
        ...
        final selectorElem = PilotWidgetInspector.findElement(
          "ElevatedButton['Proceed to Checkout']",
        );
        expect(selectorElem, isNotNull);

        // Button text (Priority 80)
        final textElem = PilotWidgetInspector.findElement(
          'Proceed to Checkout',
        );
    ...
    testWidgets(
      'Semantic Tree Compaction significantly reduces JSON payload size',
      (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
        ...
        final compactTree = PilotWidgetInspector.captureWidgetTree(
          compact: true,
        );
    ...
    test(
      'Fast similarity pre-filter handles length differences without GC pressure',
      () {
        final simExact = PilotWidgetInspector.calculateSimilarity(
          'login',
          'login',
        );
        expect(simExact, equals(1.0));

        final simClose = PilotWidgetInspector.calculateSimilarity(
          'create account',
          'create new account',
        );
        expect(simClose, greaterThan(0.65));

        final simFar = PilotWidgetInspector.calculateSimilarity(
          'a',
          'this is a completely different long string',
        );
    ...
    test('diffWidgetTrees preserves duplicate keys at different paths', () {
      final tree = {
        'type': 'Column',
        'children': [
          {'type': 'Text', 'key': 'row', 'text': 'first'},
          {'type': 'Text', 'key': 'row', 'text': 'second'},
        ],
      };
      final changed = {
        'type': 'Column',
        'children': [
          {'type': 'Text', 'key': 'row', 'text': 'updated first'},
          {'type': 'Text', 'key': 'row', 'text': 'second'},
        ],
      };

      final diff = PilotWidgetInspector.diffWidgetTrees(tree, changed);
      expect(diff['modifiedCount'], 1);
      expect(diff['modified'], contains(contains('updated first')));
    });

    testWidgets(
      'findElement matches chained (Parent -> Child) and positional selectors',
      (tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
        ...
        final chainedEl = PilotWidgetInspector.findElement(
          'card_b -> Button[\'Action Button\']',
        );
        expect(chainedEl, isNotNull);

        // Positional lookup: 2nd Action Button
        final positionalEl = PilotWidgetInspector.findElement(
          'Button[\'Action Button\'][index=1]',
        );
